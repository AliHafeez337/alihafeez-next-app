...
export async function retrieveContext(query) {
  ...
  try {
    ...
    if (queryEmbedding) {
      ...
      if (vectorResults.length && best >= CHAT_LIMITS.minSimilarity) {
        return { chunks: vectorResults, lowConfidence: false };
      }
    }
  } catch {
  ...
  if (keywordResults.length === 0) {
    const broadQuery =
      'Ali Hafeez education degree university experience skills projects work thesis PTA NET dotnet';
...
export function formatContextForPrompt(retrievedChunks) {
  ...
  return retrievedChunks
    .map(
      (c, i) =>
        `[${i + 1}] (${c.meta?.file || 'doc'} — ${c.meta?.section || 'section'})\n${c.text}`
    )
    .join('\n\n---\n\n');
}
